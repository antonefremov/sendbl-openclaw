...
2. Open <https://sendbl.com/account/tokens> and create a **personal access token** (no expiry — designed for long-running CLI/skill use).
3. Set the env var:
...
The skill will not work without `SENDBL_API_KEY`. If a token is leaked or no longer needed, revoke it from the same page — revocation takes effect immediately.

## API base URL

```
https://8naph0s2s1.execute-api.us-east-1.amazonaws.com/prod
```
...
```sh
curl -sS -X POST "$SENDBL_API/v1/requestFiles" \
...
```sh
curl -sS -X POST "$SENDBL_API/v1/sendFile" \
  -H "x-api-key: $SENDBL_API_KEY" \
  -H "content-type: application/json" \
  -d '{
...
```sh
curl -sS "$SENDBL_API/v1/uploadLinkStatus?upload_link_id=<id>"
```
...
```sh
curl -sS "$SENDBL_API/v1/uploadedFiles?token=<owner_token>&limit=50"
```

Returns the array of files with `file_id` and `filename`. To get a download URL for one of them:

```sh
curl -sS "$SENDBL_API/v1/fileDownloadLink?token=<owner_token>&file_id=<file_id>"
```
